---
title: "Outpatient Psychiatry Invisible In-Basket Work Analysis"
format: html
---

```{r setup_and_directory_creation, message=FALSE, warning=FALSE}

# Setup and Directory Creation
cat("=== SETTING UP STANDALONE REPOSITORY ===\n")

# Create directory structure
directories_to_create <- c(
  # Data storage
  "data/processed",           # Processed RDS files
  "data/raw",                # Raw data backups
  
  # Output organization
  "figures",                 # All visualization outputs
  "tables",                  # Table outputs and summaries
  "reports",                 # Final reports and documents
  "docs",                    # Documentation files
  
  # Analysis organization
  "analysis",                # Analysis results and intermediate outputs
  "scripts",                 # R scripts and utilities (if any)
...
# Save datasets
raw |> saveRDS("data/processed/01-raw-epic-data.rds")
clean |> saveRDS("data/processed/02-cleaned-epic-data.rds")
aggregated |> saveRDS("data/processed/03-aggregated-metrics.rds")
final |> saveRDS("data/processed/04-final-analysis-dataset.rds")
...
# Save table to CSV for reports
write.csv(table1, "tables/table-01-summary-by-provider-type.csv", row.names = FALSE)
...
# Save table to CSV for reports
write.csv(table2, "tables/table-02-workload-share-distribution.csv", row.names = FALSE)
...
# Save correlation matrix to CSV for reports
write.csv(cor_matrix, "tables/table-03-correlation-matrix.csv")
...
ggplot(lorenz_curve, aes(cum_prov_pct, cum_hours_pct)) +
  ...
  annotate("text", x = 0.15, y = 0.95,
           ...
           size = 5, color = "grey20", fontface = "bold", hjust = 0) +
...
ggsave('figures/figure-01-cumulative-impact-analysis.png', combined_plot, 
       width = 16, height = 12, dpi = 300, bg = 'white')

cat("Generated: figures/figure-01-cumulative-impact-analysis.png\n")
...
ggsave('figures/figure-02-workload-distribution-analysis.png', workload_plots, 
       width = 16, height = 8, dpi = 300, bg = 'white')

cat("Generated: figures/figure-02-workload-distribution-analysis.png\n")
...
ggsave('figures/figure-03-provider-type-comparison.png', combined_summary, 
       width = 12, height = 12, dpi = 300, bg = 'white')

cat("Generated: figures/figure-03-provider-type-comparison.png\n")
...
ggsave('figures/figure-04-system-overview-dashboard.png', system_overview, 
       width = 16, height = 12, dpi = 300, bg = 'white')

cat("Generated: figures/figure-04-system-overview-dashboard.png\n")
...
ggsave('figures/figure-05-inbasket-hours-distribution.png', p_enhanced_hours, 
       width = 12, height = 8, dpi = 300, bg = 'white')

cat("Generated: figures/figure-05-inbasket-hours-distribution.png\n")
...
p_lorenz <- ggplot(lorenz_curve, aes(cum_prov_pct, cum_hours_pct)) +
  ...
  # Gini coefficient - positioned in upper left area, away from curves
  annotate('text', x = 0.15, y = 0.95,
           label = paste0('Gini Coefficient = ', round(gini_coef, 3)),
           size = 5, color = 'gray20', fontface = 'bold', hjust = 0) +
  # Perfect equality label - positioned in upper right area
  annotate('text', x = 0.85, y = 0.85,
           label = 'Perfect Equality',
           size = 4, color = '#E31A1C', fontface = 'bold', hjust = 0) +
  # Bottom 50% statistic - positioned in lower center area
  annotate('text', x = 0.5, y = 0.1,
           ...
           size = 4, color = 'gray20', hjust = 0.5, fontface = 'bold') +
...
ggsave('figures/figure-06-workload-inequality-lorenz-curve.png', p_lorenz, 
       width = 10, height = 8, dpi = 300, bg = 'white')

cat("Generated: figures/figure-06-workload-inequality-lorenz-curve.png\n")
...
ggsave('figures/figure-07-correlation-matrix-heatmap.png', p_heatmap, 
       width = 10, height = 8, dpi = 300, bg = 'white')

cat("Generated: figures/figure-07-correlation-matrix-heatmap.png\n")
...
# Save comprehensive tables to CSV
write.csv(comprehensive_tables$overall_stats_table, "tables/table-04-overall-department-statistics.csv", row.names = FALSE)
write.csv(comprehensive_tables$provider_type_table, "tables/table-05-provider-type-analysis.csv", row.names = FALSE)
write.csv(comprehensive_tables$top_10_table, "tables/table-06-top-10-providers-analysis.csv", row.names = FALSE)

cat("Saved comprehensive analysis tables to tables/ directory\n")
...
report_summary <- list(
  ...
  roi = round((net_benefit / nursing_cost) * 100, 1)
)
...
visualization_files <- c(
  "figures/figure-01-cumulative-impact-analysis.png",
  "figures/figure-02-workload-distribution-analysis.png", 
  "figures/figure-03-provider-type-comparison.png",
  "figures/figure-04-system-overview-dashboard.png",
  "figures/figure-05-inbasket-hours-distribution.png",
  "figures/figure-06-workload-inequality-lorenz-curve.png",
  "figures/figure-07-correlation-matrix-heatmap.png"
)
...
validation_results$data_files <- list(
  raw_rds = file.exists("data/processed/01-raw-epic-data.rds"),
  clean_rds = file.exists("data/processed/02-cleaned-epic-data.rds"), 
  aggregated_rds = file.exists("data/processed/03-aggregated-metrics.rds"),
  final_rds = file.exists("data/processed/04-final-analysis-dataset.rds")
)
...
validation_results$financial_calculations <- list(
  ...
  roi_calculation_correct = abs(report_summary$roi - 61.3) < 0.1
)
...
```{r generate_basic_documentation, message=FALSE, warning=FALSE}

# Generate Basic Documentation Files
cat("=== GENERATING BASIC DOCUMENTATION ===\n")

# Generate README.md
readme_content <- "# Inbasketosis: Epic In-Basket Workload Analysis

## Overview
This repository contains a comprehensive analysis of Epic in-basket messaging workload across 64 healthcare providers in the Department of Psychiatry at Penn Medicine.

## Quick Start
1. **Prerequisites**: R, RStudio, Quarto
2. **Required Files**: `epic-inbasket-analysis.qmd`, `epic-signal-analytics-data.xlsx`
3. **Run Analysis**: `quarto render epic-inbasket-analysis.qmd`

## Key Findings
- **5,044× variation** in in-basket workload across providers
- **1,941.2 total hours** annually (1.04 FTE equivalent)
- **Top 10% of providers** handle 33% of total messaging burden
- **$335,828 annual revenue loss** from in-basket work
- **61.3% ROI** potential from pilot redistribution program

## Analysis Period
July 2024 – June 2025 (12 months)

## Data Source
Epic Signal Analytics, Penn Medicine Health System
...
project_structure_content <- "# Project Structure
## Inbasketosis: Epic In-Basket Workload Analysis

### Directory Organization

#### Root Files
- `epic-inbasket-analysis.qmd` - Main Quarto analysis pipeline
- `epic-signal-analytics-data.xlsx` - Raw Epic Signal Analytics data
...
opc_inbasket/
├── data/
│   ├── processed/          # Processed RDS files (01-04-*.rds)
│   └── raw/               # Raw data backups
├── figures/               # All visualization outputs (.png)
├── tables/                # Table outputs and summaries
├── reports/               # Final reports and documents
├── docs/                  # Documentation files
├── analysis/              # Analysis results and intermediate outputs
...
- **Data Files**: `data/processed/` (RDS format for reproducibility)
- **Visualizations**: `figures/` (PNG format, publication-ready)
- **Tables**: `tables/` (CSV/Excel format for reports)
- **Documentation**: `docs/` (Pipeline documentation)
- **Reports**: `reports/` (Executive summaries and final deliverables)

### File Generation
All files except `epic-inbasket-analysis.qmd` and `epic-signal-analytics-data.xlsx` are generated automatically by the pipeline.
...
writeLines(project_structure_content, "project-structure.md")
cat("Generated: project-structure.md\n")

# Generate basic pipeline documentation
pipeline_docs_content <- "# Inbasketosis Pipeline Documentation
...
writeLines(pipeline_docs_content, "docs/pipeline-documentation.md")
cat("Generated: docs/pipeline-documentation.md\n")

# Generate executive summary report
executive_summary_content <- paste0("# Executive Summary: In-Basket Workload Analysis

## Key Findings
- **Total Providers Analyzed**: ", report_summary$total_providers, "
- **Analysis Period**: July 2024 – June 2025 (12 months)
- **Total In-Basket Hours**: ", round(report_summary$total_inbasket_hours, 1), " hours (", report_summary$fte_equivalent, " FTE equivalent)
- **Total Messages Processed**: ", report_summary$total_messages, "
- **Total Appointments**: ", report_summary$total_appointments, "
- **Workload Variation**: ", report_summary$workload_variation, "× between highest and lowest providers

## Financial Impact
- **Revenue Lost to In-Basket Work**: $", format(report_summary$revenue_lost, big.mark = ","), "
- **Top 10 Providers Hours**: ", round(report_summary$top_10_hours, 1), " hours
- **Pilot Program ROI**: ", report_summary$roi, "% (", round(report_summary$net_benefit), " net annual benefit)
...
3. **Expected Outcomes**: 
   - ", report_summary$roi, "% return on investment
   - $", format(round(report_summary$net_benefit), big.mark = ","), " net annual benefit
...
writeLines(executive_summary_content, "reports/executive-summary.md")
cat("Generated: reports/executive-summary.md\n")

cat("Basic documentation files generated successfully.\n")

# All outputs are now properly organized in their designated directories
cat("Analysis outputs organized in designated directories:\n")
