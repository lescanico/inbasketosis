...
# Final calculations and aggregations
final <- clean |>
  rowwise() |>
  mutate(
    # Aggregate message counts and averages
    ...
    # Time aggregations
    count_of_weekend_minutes = sum(count_of_saturday_minutes, count_of_sunday_minutes, na.rm = TRUE),
  ...
  mutate(
    across(c(count_of_appointments,
             count_of_messages_received,
             inbasket_hours, schedule_hours, system_hours, afterhours),
      ~ sum(.x, na.rm = TRUE),
      .names = "{.col}_total"),
    .by = 1  # Create totals across all rows
  ) |>
  ...
  transmute(
    ...
    schedule_hours, system_hours, afterhours,
    
    # General load per month
    appointments_per_month = count_of_appointments_per_month,
    schedule_hours_per_month, system_hours_per_month, afterhours_per_month,
    
    # Inbasket load
    inbasket_hours,
    msg = count_of_messages_received,
    msg_pc = count_of_patient_call_messages_received,
    ...
    msg_per_month = count_of_messages_received_per_month,
    msg_pc_per_month = count_of_patient_call_messages_received_per_month,
    msg_mar_per_month = count_of_patient_medical_advice_requests_messages_received_per_month,
    msg_res_per_month = count_of_result_messages_received_per_month,
    msg_rxa_per_month = count_of_rx_auth_messages_received_per_month,
    
    # Inbasket load per appointment
    inbasket_hours_per_appointment = inbasket_hours / appointments,
    msg_per_appointment = count_of_messages_received / appointments,
    msg_pc_per_appointment = count_of_patient_call_messages_received / appointments,
    msg_mar_per_appointment = count_of_patient_medical_advice_requests_messages_received / appointments,
    msg_res_per_appointment = count_of_result_messages_received / appointments,
    msg_rxa_per_appointment = count_of_rx_auth_messages_received / appointments,
    
    # Inbasket load per scheduled hour
    inbasket_hours_per_schedule_hour = inbasket_hours / schedule_hours,
    msg_per_schedule_hour = count_of_messages_received / schedule_hours,
    msg_pc_per_schedule_hour = count_of_patient_call_messages_received / schedule_hours,
    msg_mar_per_schedule_hour = count_of_patient_medical_advice_requests_messages_received / schedule_hours,
    msg_res_per_schedule_hour = count_of_result_messages_received / schedule_hours,
    msg_rxa_per_schedule_hour = count_of_rx_auth_messages_received / schedule_hours,
    
    # Responsiveness
    avg_dtc = average_days_until_message_marked_done,
    avg_dtc_pc = average_days_until_patient_call_messages_marked_done,
    avg_dtc_mar = average_days_until_patient_medical_advice_request_message_marked_done,
    avg_dtc_res = average_days_until_result_message_marked_done,
    avg_dtc_rxa = average_days_until_rx_auth_message_marked_done,
    
    # Shares
    appointments_share = appointments / count_of_appointments_total,
    msg_share = msg / count_of_messages_received_total,
    inbasket_hours_share = inbasket_hours / inbasket_hours_total,
    schedule_hours_share = schedule_hours / schedule_hours_total,
    system_hours_share = system_hours / system_hours_total,
...
# Load data
final <- readRDS("data.rds")

# Totals
table1 <- final |>
  group_by(type) |>
  summarise(
    `n (%)` = sprintf("%d (%.1f)", n(), 100 * n() / nrow(final)),
    `Appointments - n (%)` = sprintf("%s (%.1f)", comma(sum(appointments, na.rm = TRUE)), 100 * sum(appointments, na.rm = TRUE) / sum(final$appointments, na.rm = TRUE)),
    `Schedule hours - mean (sd)` = sprintf("%.2f (%.2f)", mean(schedule_hours, na.rm = TRUE), sd(schedule_hours, na.rm = TRUE)),
    `Messages - n (%)` = sprintf("%s (%.1f)", comma(sum(msg, na.rm = TRUE)), 100 * sum(msg, na.rm = TRUE) / sum(final$msg, na.rm = TRUE))
...
avg_vars   <- c("avg_dtc","avg_dtc_pc","avg_dtc_mar","avg_dtc_res","avg_dtc_rxa")
...
cor_vars <- c(
  ...
  "avg_dtc", "avg_dtc_pc", "avg_dtc_mar", "avg_dtc_res", "avg_dtc_rxa"
)
...
provider_type_analysis <- final |>
  ...
  summarise(
    ...
    mean_response_time = mean(avg_dtc, na.rm = TRUE),
...
top_10_analysis <- final |>
  arrange(desc(inbasket_hours)) |>
  head(10) |>
  select(id, type, inbasket_hours, messages, appointments, avg_dtc, afterhours) |>
  mutate(
    type_label = case_when(
      type == 'MD' ~ 'Attending',
      type == 'NP' ~ 'Nurse Practitioner',
...
cor_vars <- c('inbasket_hours', 'messages', 'appointments', 'system_hours', 'afterhours', 'avg_dtc')
cor_data <- final |>
  select(all_of(cor_vars)) |>
  mutate(across(everything(), ~ coalesce(.x, 0)))
...
# Create comprehensive cumulative impact table
cumulative_impact_table <- cum_df |>
  filter(rank %in% c(0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 60)) |>
  mutate(
    `Top-k Providers` = rank,
    `FTE Freed` = round(fte_freed, 3),
    `Revenue Recovery ($K)` = round(revenue_recovered/1000, 1),
    `Additional Appointments` = round(added_appointments, 0),
    `After-Hours Remaining (K hrs)` = round(afterhours_remaining/1000, 1)
  ) |>
  select(`Top-k Providers`, `FTE Freed`, `Revenue Recovery ($K)`, 
         `Additional Appointments`, `After-Hours Remaining (K hrs)`)

# Add percentage calculations for key metrics
cumulative_impact_table <- cumulative_impact_table |>
  mutate(
    `% of Total FTE` = round(`FTE Freed` / max(`FTE Freed`, na.rm = TRUE) * 100, 1),
    `% of Max Revenue` = round(`Revenue Recovery ($K)` / max(`Revenue Recovery ($K)`, na.rm = TRUE) * 100, 1),
    `% of Max Appointments` = round(`Additional Appointments` / max(`Additional Appointments`, na.rm = TRUE) * 100, 1),
    `After-Hours Reduction (%)` = round((3.962 - `After-Hours Remaining (K hrs)`) / 3.962 * 100, 1)
  )

# Print the comprehensive table
cumulative_impact_table |>
  kable(format = "html", digits = c(0, 3, 1, 0, 1, 1, 1, 1, 1), 
        caption = "Cumulative Impact of In-Basket Workload Redistribution at Key Milestones") |>
  kable_styling(
    full_width = TRUE, 
    position = "left", 
    font_size = 11,
  ...
  add_header_above(c("Provider Milestone" = 1, "Resource Recovery" = 2, "Capacity Creation" = 1, 
                     "Workload Reduction" = 1, "Percentage Metrics" = 4)) |>
  footnote(
    general = c(
      "FTE calculation: 1 FTE = 1,872 hours/year (40 hours/week × 52 weeks × 0.9 efficiency)",
      "Revenue rate: $173/hour (FY2025 departmental benchmark)",
      "Appointment capacity: Based on observed throughput of 0.113 appointments/hour",
      "Baseline after-hours: 3,962 total hours annually"
    ),
    general_title = "Methodological Notes:"
  )

# Create detailed top providers impact table
top_providers_impact <- ranked |>
  head(15) |>
  mutate(
    `Rank` = row_number(),
    `Provider ID` = id,
    `Provider Type` = case_when(
      type == 'MD' ~ 'Attending Psychiatrist',
      type == 'NP' ~ 'Nurse Practitioner', 
      type == 'RF' ~ 'Resident/Fellow',
      TRUE ~ as.character(type)
    ),
    `In-Basket Hours` = round(inbasket_hours, 1),
    `Total Messages` = messages,
    `Appointments` = appointments,
    `Response Time (days)` = round(avg_dtc, 1),
    `After-Hours Work` = round(afterhours, 1),
    `FTE Equivalent` = round(inbasket_hours / (40 * 52 * 0.9), 3),
    `Revenue Recovery ($K)` = round((inbasket_hours * 173) / 1000, 1),
    `Additional Appointments` = round(inbasket_hours * (sum(final$appointments, na.rm = TRUE) / sum(final$schedule_hours, na.rm = TRUE)), 0)
  ) |>
  select(`Rank`, `Provider ID`, `Provider Type`, `In-Basket Hours`, `Total Messages`, 
         `Appointments`, `Response Time (days)`, `After-Hours Work`, `FTE Equivalent`, 
         `Revenue Recovery ($K)`, `Additional Appointments`)

# Print the detailed top providers table
top_providers_impact |>
  kable(format = "html", digits = c(0, 0, 0, 1, 0, 0, 1, 1, 3, 1, 0), 
        caption = "Top 15 Providers: Individual Impact Analysis for Workload Redistribution") |>
  kable_styling(
    full_width = TRUE, 
    position = "left", 
    font_size = 10,
  ...
  add_header_above(c("Ranking" = 1, "Provider Details" = 2, "Workload Metrics" = 3, 
                     "Performance" = 2, "Redistribution Benefits" = 3)) |>
  footnote(
    general = c(
      "Providers ranked by total in-basket hours (highest to lowest)",
      "FTE calculation based on 1,872 hours/year standard",
      "Revenue recovery assumes $173/hour opportunity cost",
      "Additional appointments based on observed throughput of 0.113 appts/hour"
    ),
    general_title = "Notes:"
  )

# Create summary metrics table for cumulative impact
summary_metrics <- data.frame(
  `Metric Category` = c(
    "Baseline Department Metrics",
    "", "", "", "",
    "Top 10 Providers (Pilot Target)",
    "", "", "", "",
    "Maximum Potential (All Providers)",
    "", "", "", "",
    "Financial Impact Analysis",
    "", "", "", ""
  ),
  `Metric` = c(
    ...
    "Total Messages", 
    "Total Appointments",
    "Baseline After-Hours",
    "Top 10 In-Basket Hours",
    "Top 10 Messages",
    "Top 10 FTE Equivalent",
    "Top 10 Revenue Recovery",
    "Top 10 After-Hours Reduction",
    "Maximum FTE Recovery",
    "Maximum Revenue Recovery", 
    "Maximum Additional Appointments",
    "Maximum After-Hours Reduction",
    "Total After-Hours Elimination",
    "Pilot Program Cost (2 FTE Nursing)",
    "Net Annual Benefit (Top 10)",
  ...
  `Value` = c(
    ...
    paste0(round(sum(final$afterhours, na.rm = TRUE), 0), " hours"),
    paste0(round(sum(top_10_providers$inbasket_hours, na.rm = TRUE), 1), " hours"),
    paste0(sum(top_10_providers$messages, na.rm = TRUE), " messages"),
    paste0(round(sum(top_10_providers$inbasket_hours, na.rm = TRUE) / (40 * 52 * 0.9), 2), " FTE"),
    paste0("$", format(sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 173, big.mark = ",")),
    paste0(round(sum(top_10_providers$afterhours, na.rm = TRUE), 0), " hours"),
    ...
    paste0(round(sum(final$inbasket_hours, na.rm = TRUE) * (sum(final$appointments, na.rm = TRUE) / sum(final$schedule_hours, na.rm = TRUE)), 0), " appointments"),
    paste0(round(sum(final$afterhours, na.rm = TRUE), 0), " hours"),
    paste0(round(sum(final$afterhours, na.rm = TRUE), 0), " hours"),
    paste0("$", format(sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 78, big.mark = ",")),
    paste0("$", format((sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 173) - (sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 78), big.mark = ",")),
    paste0(round(((sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 173) - (sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 78)) / (sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 78) * 100, 1), "%"),
    paste0(round((sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 78) / ((sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 173) - (sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 78)) * 12, 1), " months"),
    paste0(round((sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 78) / ((sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 173) - (sum(top_10_providers$inbasket_hours, na.rm = TRUE) * 78)) * 12, 1), " months")
  ),
  `Calculation Notes` = c(
    "All providers with ≥30 days in system",
    "Annual total across all providers",
    "All message types combined",
    "Annual appointment volume",
    "Work outside 7 AM–7 PM + weekends",
    "Sum of top 10 highest-volume providers",
    "Sum of top 10 highest-volume providers",
    "Top 10 hours ÷ 1,872 hours/FTE",
    "Top 10 hours × $173/hour",
    "After-hours work for top 10 providers",
    "Total hours ÷ 1,872 hours/FTE",
    "Total hours × $173/hour",
    "Total hours × 0.113 appointments/hour",
    "All after-hours work eliminated",
    "Complete after-hours elimination",
    "Top 10 hours × $78/hour nursing rate",
    "Revenue recovery - nursing costs",
    "(Net benefit ÷ nursing costs) × 100%",
    "Nursing costs ÷ net monthly benefit",
    "Time to recover initial investment"
  )
)

# Print the summary metrics table
summary_metrics |>
  kable(format = "html", 
        caption = "Cumulative Impact Analysis: Key Metrics and Financial Projections") |>
  kable_styling(
    full_width = TRUE, 
    position = "left", 
    font_size = 10,
  ...
  add_header_above(c("Category" = 1, "Metric Description" = 1, "Value" = 1, "Calculation Method" = 1)) |>
  footnote(
    general = c(
      "FTE calculation: 1,872 hours/year (40 hours/week × 52 weeks × 0.9 efficiency)",
      "Revenue rate: $173/hour (FY2025 departmental benchmark rate)",
...
cat("\nQuality Improvement Projections:\n")
cat("  Response Time Target: <24 hours (current average: ", round(mean(final$avg_dtc, na.rm = TRUE), 1), " days)\n")
